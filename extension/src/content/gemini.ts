...
async function loadContextFromLLM(sourceLLM: string) {
  try {
    ...
    if (response.success && response.data?.content) {
      ...
      // Try multiple selectors for Gemini textarea
      let textarea = document.querySelector('textarea.gds-body-l') as HTMLTextAreaElement;
      if (!textarea) {
        textarea = document.querySelector('textarea[placeholder*="Enter"]') as HTMLTextAreaElement;
      }
      if (!textarea) {
      ...
      if (!textarea) {
        textarea = document.querySelector('[contenteditable="true"]') as any;
      }
      ...
      if (textarea) {
        ...
        if (textarea.tagName === 'TEXTAREA') {
          (textarea as HTMLTextAreaElement).value = contextPrefix;
        ...
        console.error('[MCP Extension - Gemini] Textarea not found');
    ...
    console.error('[MCP Extension - Gemini] Failed to load context:', error);
...
async function injectContext() {
  ...
  try {
    ...
    console.error('[MCP Extension - Gemini] Failed to inject context:', error);
  }
}

function extractConversation(): string {
  const messages = document.querySelectorAll('.conversation-container message-content');
  let conversation = '';
...
async function saveConversation() {
  ...
  if (!conversation || conversation.length < 50) {
    showNotification('Conversation too short to save', 'warning');
    return;
  }
  ...
  try {
    ...
    console.error('[MCP Extension] Save error:', error);
  }
}
