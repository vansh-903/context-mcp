...
async function loadContextFromLLM(sourceLLM: string) {
  try {
    ...
    if (response.success && response.data?.content) {
      ...
      if (textarea) {
        ...
        if (textarea.tagName === 'TEXTAREA') {
          (textarea as unknown as HTMLTextAreaElement).value = contextPrefix;
        ...
        console.error('[MCP Extension - Claude] Textarea not found');
    ...
    console.error('[MCP Extension - Claude] Failed to load context:', error);
...
async function injectContext() {
  ...
  try {
    ...
    // Skip if no previous LLM was used
    if (!settings.lastUsedLLM) {
      return;
    }
    ...
    console.error('[MCP Extension - Claude] Failed to inject context:', error);
  }
}
...
async function saveConversation() {
  ...
  try {
    const response = await chrome.runtime.sendMessage({
      action: 'addContext',
      data: {
        content: `Claude.ai conversation:\n\n${conversation}`,
        entryType: 'summary',
        sourceLLM: LLM_TYPE,
    ...
    if (response.success) {
      ...
      console.error('[MCP Extension - Claude] Save failed:', response.error);
      showNotification('Failed to save conversation', 'error');
    }
  } catch (error) {
    showNotification('Error saving conversation', 'error');
    console.error('[MCP Extension - Claude] Save error:', error);
  }
}
